...
from runtime import StrategyMError, atomic_json, atomic_text, load_ready, managed_root, sha256_file
...
def seconds(segment: dict, side: str) -> float:
    ...
    if not isinstance(stamp, str):
        raise StrategyMError("Whisper segment has no usable timestamp")
    parts = stamp.replace(",", ".").split(":")
    if len(parts) != 3:
        raise StrategyMError("Whisper timestamp format is invalid")
...
def main() -> int:
    ...
    if not media.is_file() or not project.is_dir():
        raise StrategyMError("media file or project directory is missing")
    ...
    if sha256_file(model) != whisper["model_sha256"] or sha256_file(executable) != whisper["executable_sha256"]:
        raise StrategyMError("Whisper model or executable hash changed")
    with tempfile.TemporaryDirectory(prefix="mpi-strategy-m-media-") as directory:
        ...
        if converted.returncode:
            raise StrategyMError("FFmpeg audio extraction failed")
        ...
        if recognized.returncode:
            raise StrategyMError("whisper.cpp transcription failed")
        payload_path = output_base.with_suffix(".json")
        try:
            payload = json.loads(payload_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            raise StrategyMError("whisper.cpp did not produce valid JSON") from exc
    transcription = payload.get("transcription")
    if not isinstance(transcription, list) or not transcription:
        raise StrategyMError("Whisper transcription is empty")
    lines: list[str] = []
    segments: list[dict] = []
    ...
    for segment in transcription:
        ...
        if end <= start:
            raise StrategyMError("Whisper segment has non-positive duration")
    ...
    if not lines:
        raise StrategyMError("Whisper transcription contains no text")
