...
6. Run V4 Pro `max` as `pro_review_1`, without Flash analysis. Record
   `pro_model_1`.
...
8. In a separate Sol Medium pass, remove duplicated framing, abstract carrier
   ...
   regenerate `bilingual.dj`.
9. Run V4 Pro `max` as `pro_review_2`, again without Flash analysis. If a title,
