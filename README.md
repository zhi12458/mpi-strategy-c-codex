...
translation. M 1.0.12 fixes the model roles as V4 Flash source analysis,
...
M 1.0.12 retains cultural allusions as an audited hard gate: Flash enumerates
...
M 1.0.12 also retains the temporal/aspect, compressed-clause implicit
...
- `mpi-translations` at `96f57abefaa3d1f369a77ad791b295d9c9e40257`;
- `translation-toolkit` at `883a8c7d9df5ea971e35bde369a2279c2b29b60f`.

M 1.0.12 retains the fixed-term provenance gate: the locked toolkit
...
M 1.0.12 bounds long-document Flash requests without reducing semantic
...
batch may be checkpointed. M 1.0.12 also accepts existing 1.0.7 through 1.0.11 checkpoints
...
M 1.0.12 additionally reconciles the independently generated fields after all
...
Before a component is accepted, M 1.0.12 also verifies that every designated
...
compatible.
The same component acceptance gate now applies every complete-v3 rule that is
decidable without merging another component: Chinese analytical prose,
explicit/null/evidence consistency, temporal marker coverage, reference and
compressed-clause role checks, and constraints-internal allusion preservation.
Only the failed component receives its existing bounded retry. Cross-component
status and temporal-to-constraint checks remain at the unchanged final v3 gate.
M 1.0.12 records deterministic final validation failures without private
...
M 1.0.12 retains the previously missing schema
